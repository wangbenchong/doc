...
## 把SpriteAtlas导出为单张PNG

准备工作：PackageManager安装2D Sprite包；ProjectSettings设置 Editor > Sprite Atlas > Mode > V2 Enabled

Editor 文件夹下的脚本 `ExportSpriteAtlasToSinglePng.cs`（注释很细致的讲了颜色空间、图片导入设置的关要）：

```csharp
#define 找到正确的裁切坐标计算方法
using UnityEngine;
...
using System.Collections.Generic;
//执行前请确保ProjectSetting > Editor > Sprite Atlas > Mode设置为 V1-AlwaysEnabled 或者 V2-Enabled（推荐）
public class ExportSpriteAtlasToSinglePng
{
    [MenuItem("Assets/把SpriteAtlas导出为单张PNG")]
    static void ExportAtlas()
    {
        // 1. 加载 Sprite Atlas
        SpriteAtlas atlas = (SpriteAtlas)Selection.objects[0];
        if (atlas == null || atlas.spriteCount == 0) return;
        var atlasPath = AssetDatabase.GetAssetPath(atlas);
        bool isV2 = atlasPath.EndsWith("2");
        var atlasTexSetting = atlas.GetTextureSettings();
        //确保执行过程中保持图集的sRGB状态不变，否则导出PNG会出错。
        //避免混用，否则由sRGB改Linear会导致变亮（橙色变亮黄色）；由Linear改sRGB会导致变暗（橙色变暗红色）。
        //混用导致更灾难的情况有二：
        //1.由sRGB改Linear的结果，存入到另一个sRGB模式的图片中，会导致不可逆变亮
        //2.由Linear改sRGB的结果，存入到另一个Linear模式的图片中，会导致不可逆变暗
        //总之避免混用，原则上应始终保持sRGB，除非记录HDR信息。
        //题外话：色彩空间的总配置在ProjectSetting > Player > Other Settings > Color Space，URP管线默认Linear。
        bool isSRGB = atlasTexSetting.sRGB;
        ...
        Texture2D atlasTexture = previews[0];

        // 3. 导出为 PNG
        string exportPath = atlasPath.Replace(isV2 ? ".spriteatlasv2" : ".spriteatlas", ".png");   
        //atlasTexture.EncodeToPNG() 要求太多了，使用的每个散图必需无压缩且可读。尝试用RenderTexture来代替处理
        Texture2D readableTex = DuplicateTexture(atlasTexture, isSRGB);
        byte[] bytes = readableTex.EncodeToPNG();
        File.WriteAllBytes(exportPath, bytes);
        AssetDatabase.ImportAsset(exportPath); // 触发导入流程

        // 4. 配置 TextureImporter 裁切信息
        ConfigureTextureImporter(exportPath, atlas, isV2, isSRGB);
    }
    [MenuItem("Assets/把SpriteAtlas导出为单张PNG", true)]
    static bool Validate_spriteatlas()
    {
        Object[] objs = Selection.objects;
        if(objs == null) return false;
        for (int i = 0, length = objs.Length; i < length && i < 1; i++)
        {
            if(objs[i] != null && objs[i] is SpriteAtlas)
            {
                var sa = (SpriteAtlas)objs[i];
                if(sa.spriteCount > 0) return true;
            }
        }
        return false;
    }
    private static Texture2D DuplicateTexture(Texture2D source, bool isSRGB) {
		...
		Texture2D readableText = new Texture2D(source.width, source.height, TextureFormat.ARGB32, false);
    ...
    static void ConfigureTextureImporter(string pngPath, SpriteAtlas atlas, bool isV2, bool isSRGB)
    {
        //如果类继承AssetPostprocessor就在OnPreprocessTexture()函数里这么写：var importer = (TextureImporter)assetImporter;
        //反之，按如下获取 TextureImporter
        var importer = AssetImporter.GetAtPath(pngPath) as TextureImporter;
        if (importer == null) return;

        //有些设置，比如generate physics shape需要写TextureImporterSettings，按需使用
        ...
        if(EditorApplication.isPlaying)
        {
            SpriteRect[] rectList = new SpriteRect[atlas.spriteCount];
            ...
            for (int i = 0; i < sprites.Length; i++)
            {
                ...
                spriteRect.rect = sprite.textureRect;
                spriteRect.pivot = sprite.pivot;
                spriteRect.border = sprite.border;

                var tempName = sprite.name.Replace("(Clone)", "").Trim();
                if(dic.Contains(tempName))
                {
                    tempName = tempName + "_" + i;
                }
                dic.Add(tempName);
                spriteRect.name = tempName;
                rectList[i] = spriteRect;
            }
            var factory = new SpriteDataProviderFactories();
            factory.Init();
            var dataProvider = factory.GetSpriteEditorDataProviderFromObject(importer);
            dataProvider.InitSpriteEditorDataProvider();
            //如果之前没有 SpriteRect[] 数据,就先获取已有的 var rectList = dataProvider.GetSpriteRects();
            dataProvider.SetSpriteRects(rectList);
            dataProvider.Apply();
            //importer.spritesheet = metaDataList;//旧写法
        }
        else
        {
            EditorUtility.DisplayDialog("警告","虽然生成了png，但裁切信息导出失败！如果需要导出带裁切信息的png文件，请在运行时再次尝试。","好的");
        }
        #endif
        //如果类没有继承AssetPostprocessor,就需要手动调用SaveAndReimport()
...
[Github：RightClickMenuExtension · sunsvip/GF_X](https://github.com/sunsvip/GF_X/blob/master/Assets/AAAGame/ScriptsBuiltin/Editor/Common/RightClickMemus/RightClickMenuExtension.SpriteAtlasTools.cs)
...
Editor 文件夹下的脚本 `ExportSpriteSheetsToMultiPng.cs`：
...
using UnityEditor.U2D.Sprites;
using System.Collections.Generic;
public class ExportSpriteSheetsToMultiPng
{
    [MenuItem("Assets/把Sprite导出为多张PNG散图")]
    static void ExportSprites()
    {
        ...
        List<bool> isSRGB = new List<bool>();
        ...
        for (int i = 0; i < selectAssetsCount; i++)
        {
            ...
            var outputDir = $"{Path.GetDirectoryName(spFileName)}/{Path.GetFileNameWithoutExtension(spFileName)}_sliced";
            ...
            //旧写法：var spRects = texImporter.spritesheet;
            ...
            for (int spIndex = 0; spIndex < childrenSpCount; spIndex++)
            {
                ...
                tex.Apply();
                string fileName = $"{outputDir}/{spDt.name}.png";
                ...
                isSRGB.Add(texImporter.sRGBTexture);
                spriteRects.Add(spDt);
            }
            texImporter.isReadable = texReadable;
            texImporter.SaveAndReimport();
        }
        AssetDatabase.Refresh();
        for(int i=0;i<slicedSpritesAssets.Count;i++)
        {
            var item = slicedSpritesAssets[i];
            ...
            texImporter.sRGBTexture = isSRGB[i];
            texImporter.spriteBorder = spriteRects[i].border;
            texImporter.spritePivot = spriteRects[i].pivot;
            texImporter.SaveAndReimport();
        }
        EditorUtility.ClearProgressBar();
    }
    ...
    [MenuItem("Assets/把Sprite导出为多张PNG散图", true)]
    static bool Validate_spriteatlas()
    {
        Object[] objs = Selection.objects;
        if(objs == null) return false;
...
参考文章：[【Unity编辑器扩展】SpriteAltas资源一键转换为TMP_SpriteAsset或Sprite图集_tmp sprite asset-CSDN博客](https://blog.csdn.net/final5788/article/details/141818775)
